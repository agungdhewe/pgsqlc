...
async function main() {
	...
	try {
		...
		const rows = [
			{ user_id: 54, user_nickname: 'agung', user_fullname: 'Agung Nugroho', user_email: 'agung_dhewe@yahoo.com', _modifyby:1, _modifydate:'now()' },
			{ user_id: 55, user_nickname: 'budi', user_fullname: 'Budi Santoso', user_email: 'budi_santoso@gmail.com' , _modifyby:1, _modifydate:'now()'},
		...
		for (var row of rows) {
			if (cmdDelete==null) {
				cmdDelete = sqlUtil.createDeleteCommand(tablename, ['user_id'])
			}
			var result = await cmdDelete.execute(row)
			console.log(result)
		}
