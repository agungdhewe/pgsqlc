...
async function main() {
	...
	try {
		...
		const rows = [
			{ user_nickname: 'agung', user_fullname: 'Agung Nugroho', user_email: 'agung_dhewe@yahoo.com', _createby: 1 },
			{ user_nickname: 'budi', user_fullname: 'Budi Santoso', user_email: 'budi_santoso@gmail.com', _createby: 1 },
		...
		for (var row of rows) {
			if (cmdInsert==null) {
				cmdInsert = sqlUtil.createInsertCommand(tablename, row, ['user_id'])
			}
			var result = await cmdInsert.execute(row)
			console.log(result)
		}
